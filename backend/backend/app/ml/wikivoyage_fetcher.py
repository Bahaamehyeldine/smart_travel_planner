...
- Rate limiting with random jitter respects Wikivoyage as a free community resource
...
# Random jitter between requests — looks less robotic to Cloudflare
REQUEST_DELAY_MIN = 4.0
REQUEST_DELAY_MAX = 7.0

# MediaWiki API etiquette requires a descriptive User-Agent
HEADERS = {
...
def fetch_article(destination: str, force_refresh: bool = False) -> dict:
    ...
    try:
        ...
        # Random delay looks less robotic to Cloudflare rate limiting
        # Sleep AFTER caching so interrupt doesn't lose fetched data
